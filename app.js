...
function fillSelects(){optionize($('fromBranch'),FROM_BRANCHES); optionize($('toBranch'),BRANCHES); ['escort','driver','guard','requester','receiver'].forEach(role=>{const id=role==='escort'?'escortId':role==='driver'?'driverId':role==='guard'?'guardId':role==='requester'?'requesterId':'receiverId'; const el=$(id); if(el) optionize(el,personOptions(role));});}
...
function shouldPrintFundReplenish13(d){
  return isProvincialOrigin(d.fromBranch) && d.toBranch === 'Agribank CN Thọ Xuân Thanh Hoá' && onlyCashFxNoImportantGoods(d);
}
function shouldPrintAcqtCashProposal(d){
  return isProvincialOrigin(d.fromBranch) && d.toBranch === 'Agribank CN Thọ Xuân Thanh Hoá' && acqtTableRows(d).length;
}
function shouldPrintAcqtIntroduction(d){
  return isProvincialOrigin(d.fromBranch) && acqtTableRows(d).length;
}
